...
public class DefaultFuture {
	...
	public final static ConcurrentHashMap<Long, DefaultFuture> map = new ConcurrentHashMap<Long, DefaultFuture>();
	...
	public static void receive(Response response) {
		if (response != null) {
			DefaultFuture future = map.get(response.getId());
			if (future != null) {
	...
	static class ClearFutureThread extends Thread {
		...
		public void run() {
			...
			for (Long id : ids) {
				DefaultFuture f = map.get(id);
				if (f == null) {
					map.remove(f);
				} else if (f.getTimeOut() < (System.currentTimeMillis() - f.getStart())) {// 链路超时
